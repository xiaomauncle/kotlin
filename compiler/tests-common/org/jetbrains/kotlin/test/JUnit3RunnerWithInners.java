...
/**
 * This runner runs class with all inners test classes, but monitors situation when those classes
 * are planned to be executed with IDEA package test runner.
 */
public class JUnit3RunnerWithInners extends Runner implements Filterable, Sortable {
    ...
    private void initialize() {
        ...
        delegateRunner = new JUnit38ClassRunner(collectedTests) {
            @Override public void filter(Filter filter) throws NoTestsRemainException {
                ...
                String classPattern = getGradleClassPattern(filter);

                if (classPattern != null) {
                    if (Pattern.compile(classPattern + "\\$.*").matcher(classDescription).matches()) {
                        return;
                    }
                }
    ...
    private static String getGradleClassPattern(Filter filter) {
        try {
            ...
            if (!"org.gradle.api.internal.tasks.testing.junit.JUnitTestClassExecuter$MethodNameFilter"
                    .equals(filterClass.getName())) {
                return null;
            }
            ...
            if (!"org.gradle.api.internal.tasks.testing.filter.TestSelectionMatcher".equals(
                    testSelectionMatcherClass.getName())) {
                return null;
            }

            Field includePatternsField = testSelectionMatcherClass.getDeclaredField("includePatterns");
